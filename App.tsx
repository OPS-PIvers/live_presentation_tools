...
import { Tool, CanvasTransform, SpotlightState, ClickRecord, Slide } from './types';
...
const App: React.FC = () => {
  ...
  const [replayCursor, setReplayCursor] = useState<{ x: number; y: number; slideIndex: number } | null>(null);

  const addSlide = () => {
    setSlides(prev => [...prev, { ...createEmptySlide(), id: Date.now() + prev.length }]);
  ...
  useKeyboardShortcuts(setActiveTool, toggleCapture, goToNextSlide, goToPrevSlide);
    ...
    if (file) {
      ...
      if (type) {
        setSlides(prevSlides => {
          ...
          if (slideToUpdate.mediaUrl) URL.revokeObjectURL(slideToUpdate.mediaUrl);
          newSlides[slideIndex] = { ...slideToUpdate, mediaUrl: url, mediaType: type };
          return newSlides;
        });
      } else {
  ...
  const handleRecordClick = (x: number, y: number, transform: CanvasTransform, spotlight: SpotlightState | null) => {
    ...
    setClickSequence(prev => [
      ...prev,
      { id: Date.now(), slideIndex: currentSlideIndex, x, y, toolState: { transform, spotlight } }
    ]);
  };
  ...
  useEffect(() => {
    ...
    const animateStep = async () => {
        ...
        setReplayCursor({ x: record.x, y: record.y, slideIndex: record.slideIndex });
        await sleep(800);
        if (isCancelled) return;
    ...
    setSlides(prev => prev.map(slide => ({ ...slide, transform: INITIAL_TRANSFORM, spotlight: null })));
            ...
            {slides.map((slide, index) => (
                <div key={slide.id} className="w-full h-full flex-shrink-0">
                    <Canvas 
                        ...
                        isPlaying={isPlaying}
                        replayCursorPos={replayCursor?.slideIndex === index ? {x: replayCursor.x, y: replayCursor.y} : null}
                        onTransformChange={(transform) => updateSlideState(index, { transform })}
                        onSpotlightChange={(spotlight) => updateSlideState(index, { spotlight })}

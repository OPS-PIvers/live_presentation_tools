...
import { ZoomIcon, SpotlightIcon, CaptureIcon, PlayIcon, ResetIcon, PlusIcon } from './icons';
...
const ToolButton: React.FC<{
  ...
  disabled?: boolean;
  title: string;
}> = ({ label, shortcut, icon, isActive = false, isToggle = false, onClick, disabled = false, title }) => {
  const baseClasses = "flex flex-col items-center justify-center p-2 rounded-lg transition-all duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100";
  ...
  return (
    <button onClick={onClick} disabled={disabled} className={`${baseClasses} ${isActive ? activeClasses : inactiveClasses}`} title={finalTitle}>
      {icon}
...
export const Toolbar: React.FC<ToolbarProps> = ({ 
    activeTool, setActiveTool, onReplay, onReset, hasSequence, isCapturing, toggleCapture, onAddSlide, currentSlideIndex, totalSlides 
}) => {
  return (
